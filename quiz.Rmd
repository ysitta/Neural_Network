...
___
1. After the data has been reshaped to an array, you must continue the pre-processing step before training the network. If you inspect an arbitrary image in the training set, you will see that the pixel values fall in the range of 0 to 255. What is the purpose of dividing the array with the value 255?
...
Because the predicted label is still in dbl type, please decode the label based on its categories. Run the following code to create `decode()` function.
